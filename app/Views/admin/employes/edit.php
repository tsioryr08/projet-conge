  ...
  <div class="main">
    <div class="topbar">
      <div>
        ...
        <div class="topbar-breadcrumb"><a href="/admin">Admin</a> <i class="bi bi-chevron-right"></i> <a href="/admin/employes">Employés</a> <i class="bi bi-chevron-right"></i> Éditer</div>
      </div>
    </div>
    ...
    <div class="content">
      ...
      <div class="data-card" style="max-width:600px">
        <form method="post" action="/admin/employes/<?= esc($employe['id']) ?>">
          <?= csrf_field() ?>

          <div class="form-group">
            <label class="f-label">Nom *</label>
            <input type="text" name="nom" class="f-input" required value="<?= esc($employe['nom'] ?? '') ?>">
          </div>

          <div class="form-group">
            <label class="f-label">Prénom *</label>
            <input type="text" name="prenom" class="f-input" required value="<?= esc($employe['prenom'] ?? '') ?>">
          </div>

          <div class="form-group">
            <label class="f-label">Email *</label>
            <input type="email" name="email" class="f-input" required value="<?= esc($employe['email'] ?? '') ?>">
          </div>
          ...
          <div class="form-group">
            ...
            <select name="departement_id" class="f-select" required>
              ...
              <?php foreach ($departements as $dept): ?>
                <option value="<?= esc($dept['id']) ?>" <?= ($employe['departement_id'] ?? null) == $dept['id'] ? 'selected' : '' ?>>
                  <?= esc($dept['nom'] ?? '') ?>
                </option>
              <?php endforeach; ?>
            </select>
          ...
          <div class="form-group">
            ...
            <select name="role" class="f-select" required>
              <option value="employe" <?= ($employe['role'] ?? null) === 'employe' ? 'selected' : '' ?>>Employé</option>
              <option value="rh" <?= ($employe['role'] ?? null) === 'rh' ? 'selected' : '' ?>>RH</option>
              <option value="admin" <?= ($employe['role'] ?? null) === 'admin' ? 'selected' : '' ?>>Admin</option>
            </select>
          </div>

          <div class="form-group">
            <label class="f-label">Date d'embauche *</label>
            <input type="date" name="date_embauche" class="f-input" required value="<?= esc($employe['date_embauche'] ?? '') ?>">
          </div>

          <div class="form-group">
            <label class="f-label">Statut *</label>
            <select name="actif" class="f-select" required>
              <option value="1" <?= ($employe['actif'] ?? 1) == 1 ? 'selected' : '' ?>>Actif</option>
              <option value="0" <?= ($employe['actif'] ?? 1) == 0 ? 'selected' : '' ?>>Inactif</option>
            </select>
          </div>
