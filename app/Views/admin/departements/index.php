  ...
  <div class="main">
    <div class="topbar">
      <div>
        ...
        <div class="topbar-breadcrumb"><a href="/admin">Admin</a> <i class="bi bi-chevron-right"></i> Départements</div>
      </div>
    ...
    <div class="content">
      ...
      <div class="form-section">
        ...
        <form method="post" action="/admin/departements" style="margin-bottom:0">
          <?= csrf_field() ?>
          <div class="form-grid-2" style="margin-bottom:1rem">
            <div class="f-group">
              <label class="f-label">Nom *</label>
              <input type="text" name="nom" class="f-input" placeholder="IT" value="<?= old('nom') ?>" required/>
            </div>
            <div class="f-group">
              <label class="f-label">Description</label>
              <input type="text" name="description" class="f-input" placeholder="Informatique et technologie" value="<?= old('description') ?>"/>
            </div>
          </div>
      ...
      <div class="data-card">
          ...
          <table class="tbl">
            ...
            <tbody>
              <?php foreach ($departements as $dept): ?>
                <tr>
                  <td class="td-name">
                    <div style="font-weight:500"><?= esc($dept['nom'] ?? '') ?></div>
                  </td>
                  <td><?= esc($dept['description'] ?? 'N/A') ?></td>
                  <td class="td-actions">
                    <a href="/admin/departements/<?= esc($dept['id']) ?>/edit" class="btn-icon" title="Éditer">
                      <i class="bi bi-pencil"></i>
                    </a>
                    <form method="post" action="/admin/departements/<?= esc($dept['id']) ?>" style="display:inline" onsubmit="return confirm('Supprimer ce département ?')">
                      <?= csrf_field() ?>
        ...
        <?php else: ?>
          <div style="padding:2rem;text-align:center;color:#666;">Aucun département</div>
        <?php endif; ?>
      </div>
