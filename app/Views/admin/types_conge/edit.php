...
<div class="app-wrap">
  ...
  <div class="main">
    <div class="topbar">
      <div>
        <div class="topbar-title">Éditer un type de congé</div>
        <div class="topbar-breadcrumb"><a href="/admin">Admin</a> <i class="bi bi-chevron-right"></i> <a href="/admin/types_conge">Types de congé</a> <i class="bi bi-chevron-right"></i> Éditer</div>
      </div>
    </div>
    ...
    <div class="content">
      ...
      <div class="data-card" style="max-width:600px">
        <form method="post" action="/admin/types_conge/<?= esc($type['id']) ?>">
          <?= csrf_field() ?>

          <div class="form-group">
            <label class="f-label">Libellé *</label>
            <input type="text" name="libelle" class="f-input" required value="<?= esc($type['libelle'] ?? '') ?>">
          </div>

          <div class="form-group">
            <label class="f-label">Jours annuels *</label>
            <input type="number" name="jours_annuels" class="f-input" required min="1" value="<?= esc($type['jours_annuels'] ?? '') ?>">
          </div>

          <div class="form-group">
            <label class="f-label">Déductible *</label>
            <select name="deductible" class="f-select" required>
              <option value="1" <?= ($type['deductible'] ?? null) == 1 ? 'selected' : '' ?>>Oui</option>
              <option value="0" <?= ($type['deductible'] ?? null) == 0 ? 'selected' : '' ?>>Non</option>
            </select>
          </div>

          <div style="display:flex;gap:1rem;margin-top:2rem">
            <button type="submit" class="btn-forest">Mettre à jour</button>
            <a href="/admin/types_conge" class="btn-secondary">Annuler</a>
          </div>
        </form>
      </div>
